...
function jitter(h: Hospital): Hospital {
  const noise = Math.random() * 6 - 3; // -3 to +3
  const demand = Math.max(10, Math.min(98, h.demand + noise));
  const available = Math.max(0, Math.round(h.total * (1 - demand / 100)));
...
function zoneColor(demand: number): {
  ...
  if (demand >= 80)
    return { fill: "#c0392b", stroke: "#8f1d1d", label: "CRITICAL" };
  if (demand >= 65)
    return { fill: "#e67e22", stroke: "#c47a1d", label: "HIGH" };
  if (demand >= 45)
    return { fill: "#f1c40f", stroke: "#b7950b", label: "MODERATE" };
...
export default function HeatMapPage() {
  ...
  useEffect(() => {
    const load = async () => {
      let data = await getHeatmap().catch(() => []);

      // 🛑 filter broken backend data
      const cleanBackend = data
      ...
      // 🔥 always include fallback (with jitter)
      const fake = FALLBACK_HOSPITALS.map(jitter);

      // 🧠 merge (backend first, fallback fills gaps)
      const merged = [...cleanBackend, ...fake];
  ...
  const highCount = hospitals.filter(
    (h) => h.demand >= 65 && h.demand < 80,
  ).length;
  const modCount = hospitals.filter(
    (h) => h.demand >= 45 && h.demand < 65,
  ).length;
      ...
      <header className="heatmap-header">
        <div className="heatmap-header-left">
          ...
          <p className="heatmap-sub">
            Live resource utilisation · 10-second refresh
          </p>
        </div>
      ...
      <div className="heatmap-body">
                    ...
                    >
                      <Tooltip direction="top" offset={[0, -10]} opacity={0.95}>
                        <div className="map-tooltip">
                          ...
                          <span>
                            {h.demand}% utilised · {h.available} beds free
                          </span>
                        </div>
                      </Tooltip>
                    </CircleMarker>
        ...
        <div className="info-panel">
          ...
          <div className="hospital-list">
            {hospitals
              .filter(
                (h) => filter === "ALL" || zoneColor(h.demand).label === filter,
              )
                ...
                return (
                  ...
                  >
                    <div className="hosp-row-left">
                      <span
                        className="hosp-demand-dot"
                        style={{ background: fill }}
                      />
                      <span className="hosp-name">{h.name}</span>
                    </div>
                    <span className="hosp-demand-val" style={{ color: fill }}>
                      {h.demand}%
                    </span>
                  </div>
                );
            ...
            (() => {
              ...
              return (
                <div
                  ...
                  style={{
                    borderColor: stroke,
                    boxShadow: `6px 6px 0 ${stroke}`,
                  }}
                >
                  <div
                    className="info-card-header"
                    style={{ borderBottomColor: stroke }}
                  >
                    <h3>{selected.name}</h3>
                    <span
                  ...
                  <div className="info-metric">
                    ...
                    <span className="metric-val" style={{ color: fill }}>
                      {selected.demand}%
                    </span>
                  </div>
                  ...
                  <div className="info-grid">
                    ...
                    <div className="info-metric-sm">
                      ...
                      <span className="metric-val-sm" style={{ color: fill }}>
